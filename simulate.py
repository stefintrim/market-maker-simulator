    ...
    def calculatePurchaseReturn(self, etherAmount):
        ...
        if self.supply < self.firstSupply:
            ...
            if  exteraEtherAmount > 0:
                ...
                result = ((exteraEtherAmount+self.firstReserve-self.reserveShiftAmount)/(self.firstReserve-self.reserveShiftAmount))**(self.cw)
            ...
            baseSupply = self.supply
            result = ((etherAmount+self.reserve-self.reserveShiftAmount)/(self.reserve-self.reserveShiftAmount))**(self.cw)
    ...
    def calculateSaleReturn(self, tokenAmount):
        if self.supply > self.firstSupply:
            ...
            if exteraTokenAmount > 0:
                ...
                etherAmount += self.firstReserve * (exteraTokenAmount/self.firstSupply)
                
            else:
                etherAmount = (self.reserve-self.reserveShiftAmount) * (1- ( (1 - tokenAmount/self.supply)**(1/self.cw) ) )
...
def test():
    ...
    token = 5000
    for i in range(10):
        eth = bancor.sell(token)
        print('sell', token, eth)
